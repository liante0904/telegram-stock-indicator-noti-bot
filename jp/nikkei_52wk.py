...
def get_nikkei225_symbols_from_naver():
    ...
    try:
        ...
        symbols = [f"{stock['symbolCode']}.T" for stock in stocks]  # '.T' 접미사 추가
        print(f"총 {len(symbols)}개의 NIKKEI225 종목을 가져왔습니다.")
        return symbols
...
# 네이버 API를 이용해 52주 신고가 정보 & 일단위 캔들차트 이미지 가져오기
async def find_52_week_high_from_naver(ticker):
    url = f"https://api.stock.naver.com/stock/{ticker}/basic"
    try:
        response = requests.get(url)
        response.raise_for_status()
        data = response.json()
        # "stockItemTotalInfos"에서 52주 최고가 정보를 가져오기
        item_info = data.get("stockItemTotalInfos", [])
        high_52_weeks = None
        current_price = float(data.get("closePrice").replace(",", ""))

        for item in item_info:
            if item.get("key") == "52주 최고":  # 52주 최고가 데이터 찾기
                high_52_weeks = float(item.get("value").replace(",", ""))  # 쉼표 제거 후 float 변환

        print(current_price, high_52_weeks)
        if high_52_weeks and current_price:
            # 현재가가 52주 최고가와 같은지 확인
            return np.isclose(current_price, high_52_weeks)
        else:
            print(f"{ticker}: 52주 최고가 또는 현재가 정보가 부족합니다.")
            return False
    except requests.RequestException as e:
...
async def analyze_nikkei225():
    print("nikkei225 종목 목록을 네이버 API에서 가져오는 중...")
    str_msg = ''
    nikkei225_tickers = get_nikkei225_symbols_from_naver()
    if not nikkei225_tickers:
        print("nikkei225 목록을 가져오지 못했습니다.")
        return
    ...
    with tqdm(total=len(nikkei225_tickers_with_names), desc="Analyzing", unit="stock") as pbar:
        for idx, (ticker, name) in enumerate(nikkei225_tickers_with_names):
            # 네이버 API를 이용한 52주 신고가 확인
            is_high = await find_52_week_high_from_naver(ticker)
            pbar.update(1)  # 진행 상황 업데이트
            print(f"Analyzing {ticker} ({idx + 1} of {len(nikkei225_tickers)})...")
            if not is_high:
